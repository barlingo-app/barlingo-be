...
public class UserDiscountServiceImpl implements IUserDiscountService {
	...
	public UserDiscount save(UserDiscount userDiscount) {
		...
		Assert.isTrue(this.languageExchangeService.findById(userDiscount.getLangExchange().getId()) != null,
				"discount exchange doesnt exist");
		/*
		 * Comprueba que el usuario del descuento se haya unido al intercambio del
		 * descuento
		 */
	...
	public UserDiscount validate(UserDiscount userDiscount) {
		...
		Assert.isTrue(userDiscount.getVisible(), "discount code is not visible yet");

		// Check code has not expired
		Assert.isTrue(userDiscount.getLangExchange().getMoment().plusSeconds(86400).isAfter(LocalDateTime.now()),
